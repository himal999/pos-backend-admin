...
public class SectionSpecification {
    public static Specification<WebSection> getSpecification(CommonSearchDTO filterDto) {
        log.info("Section filter: " + filterDto);
    ...
    public static Specification<WebSection> getSpecification() {
        log.info("Section filter default :");
        return (root, query,criteriaBuilder) -> {
            List<Predicate> predicates = new ArrayList<>();
            predicates.add(root.get("status").in(List.of(Status.ACTIVE, Status.INACTIVE)));
            return criteriaBuilder.and(predicates.toArray(new Predicate[0]));
