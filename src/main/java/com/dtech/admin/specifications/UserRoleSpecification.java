...
public class UserRoleSpecification {
    public static Specification<WebUserRole> getSpecification(CommonSearchDTO filterDto) {
        log.info("User role filter: " + filterDto);
    ...
    public static Specification<WebUserRole> getSpecification() {
        log.info("User role filter default :");
        return (root, query,criteriaBuilder) -> {
            List<Predicate> predicates = new ArrayList<>();
            predicates.add(root.get("status").in(List.of(Status.ACTIVE, Status.INACTIVE)));
            return criteriaBuilder.and(predicates.toArray(new Predicate[0]));
