...
public class PageSpecification {
    public static Specification<WebPage> getSpecification(CommonSearchDTO filterDto) {
        log.info("Page filter: " + filterDto);
    ...
    public static Specification<WebPage> getSpecification() {
        log.info("Page filter default :");
        return (root, query,criteriaBuilder) -> {
            List<Predicate> predicates = new ArrayList<>();
            predicates.add(root.get("status").in(List.of(Status.ACTIVE, Status.INACTIVE)));
            return criteriaBuilder.and(predicates.toArray(new Predicate[0]));
